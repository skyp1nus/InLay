...
### Рекомендація: поетапно, з відкритими дверима

**Фаза 1 (зараз → 1.0).** Лишити **MIT** (файл уже в репозиторії — добре). Це максимізує головне на цьому етапі — зірки, winget, згадки, контриб'юторів, репутацію. PowerToys-аудиторія любить MIT.

**Критичне правило вже зараз:** опубліковані під MIT версії відкликати неможливо, але **майбутні** версії ти маєш право переліцензувати — *поки ти єдиний правовласник коду*. Тому з **першого ж зовнішнього PR** підключи CLA (наприклад, cla-assistant): без підписаних CLA переліцензування пізніше вимагатиме згоди кожного контриб'ютора і фактично стане неможливим. CLA — це страховка, яка тримає всі моделі B–E відкритими.

**Фаза 2 (після 1.0, коли є трафік).** Обрати за фактичною метою:

- Якщо головне — **популярність і портфоліо**, а гроші приємний бонус → модель **A**: платний Store-листинг ($5–10) + Sponsors + trademark. Найчистіший «повноцінний opensource».
- Якщо головне — **продавати ліцензії як Mole** → модель **B/D-гібрид**: `InLay.Core` + `InLay.Diag` лишаються MIT (спільнота, зірки, довіра), а сам застосунок з 1.x переходить під **FSL-1.1** з license key (~$10–15 one-time, 2–3 ПК, lifetime updates). FSL елегантно пом'якшує «ефект Burrow» (комерційні клони заборонені) і водночас чесний зі спільнотою: кожна версія за 2 роки стає MIT.

**Незалежно від моделі:**

- **Trademark на «InLay» + лого** — головний важіль (модель Files/Firefox/VS Code): код може бути вільним, бренд — ні. Спершу перевірити зайнятість імені у своїх юрисдикціях і в Store.
- **Платіжка** — merchant of record (Paddle / Lemon Squeezy / Dodo Payments): вони самостійно платять VAT/податки з продажів по світу — критично для одноосібного розробника.
- **Валідація ліцензій** — офлайн, підписані ключі (ed25519), без телеметрії. Довіра — частина продукту.
